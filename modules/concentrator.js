...
export class Concentrator {
	...
	static _onReady() {
		...
		if (!conditionExists) {
			console.log(
				`${game.i18n.localize(`${NAME}.CONCENTRATOR.MessagePrefix`)} ${game.i18n.localize(
					`${NAME}.CONCENTRATOR.NoMatchingCondition`
				)}`
			);
	...
	static async _onRenderChatMessage(app, html, data) {
		...
		const isConcentration = concentrationDiv.length
			? true
			: isSpell
			? !!getProperty(item, `system.components.concentration`)
			: false;
	...
	static _onPreUpdateActor(actor, update, options, userId) {
		...
		const newTempHealth = getProperty(
			update,
			`system.${Sidekick.getSetting(SETTING_KEYS.concentrator.healthAttribute)}.temp`
		);
		const oldTempHealth = getProperty(
			actor,
			`system.${Sidekick.getSetting(SETTING_KEYS.concentrator.healthAttribute)}.temp`
		);
		const tempDamageTaken = Concentrator._wasDamageTaken(newTempHealth, oldTempHealth);

		const newHealth = getProperty(
			update,
			`system.${Sidekick.getSetting(SETTING_KEYS.concentrator.healthAttribute)}.value`
		);
		const oldHealth = getProperty(
			actor,
			`system.${Sidekick.getSetting(SETTING_KEYS.concentrator.healthAttribute)}.value`
		);
		...
		if (tempDamageTaken || damageTaken) {
			...
			options[NAME][FLAGS.concentrator.damageAmount] = tempDamageTaken
				? Concentrator._calculateDamage(newTempHealth, oldTempHealth)
				: Concentrator._calculateDamage(newHealth, oldHealth);
	...
	static _onPreUpdateToken(token, update, options, userId) {
		...
		const newTempHealth = getProperty(
			update,
			`actorData.system.${Sidekick.getSetting(SETTING_KEYS.concentrator.healthAttribute)}.temp`
		);
		const oldTempHealth = getProperty(
			token,
			`actor.system.${Sidekick.getSetting(SETTING_KEYS.concentrator.healthAttribute)}.temp`
		);
		...
		const newHealth = getProperty(
			update,
			`actorData.system.${Sidekick.getSetting(SETTING_KEYS.concentrator.healthAttribute)}.value`
		);
		const oldHealth = getProperty(
			token,
			`actor.system.${Sidekick.getSetting(SETTING_KEYS.concentrator.healthAttribute)}.value`
		);
		...
		if (tempDamageTaken || damageTaken) {
			...
			cubOption[FLAGS.concentrator.damageAmount] = tempDamageTaken
				? Concentrator._calculateDamage(newTempHealth, oldTempHealth)
				: Concentrator._calculateDamage(newHealth, oldHealth);
		...
		let owners = game.users.contents.filter(
			(user) =>
				...
				actor.testUserPermission(
					user,
					Sidekick.getKeyByValue(CONST.DOCUMENT_PERMISSION_LEVELS, CONST.DOCUMENT_PERMISSION_LEVELS.OWNER)
				) &&
				!user.isGM
		);
	...
	static async _displayPrompt(uuid, userId, dc) {
		...
		new Dialog({
			...
			content: game.i18n.format(`${NAME}.CONCENTRATOR.Prompts.Check.Content`, {
	...
	static _processConcentrationCheck(event, actor, dc) {
		...
		Hooks.once("createChatMessage", (message, options, userId) => {
			const includesSaveText = message.flavor?.includes(
				game.i18n.format("DND5E.SavePromptTitle", { ability: CONFIG.DND5E.abilities[ability] })
			);
	...
	static _cancelPrompt(userId) {
		...
		const dialog = Object.values(ui.windows)?.find(
			(w) => w.title === game.i18n.localize(`${NAME}.CONCENTRATOR.Prompts.Check.Title`)
		);
		...
		ui.notifications.notify(
			`${game.i18n.localize(`${NAME}.SHORT_NAME`)} | ${game.i18n.localize(
				`${NAME}.CONCENTRATOR.Prompts.Check.ClosedByOther`
			)}`
		);
	}
	...
	static _displayChat(
		type,
		entity,
		{ dc = 0, newSpell = { name: game.i18n.localize(`${NAME}.CONCENTRATOR.UnknownSpell`), id: "" } } = {}
	) {
		...
		const speaker = isActor
			? ChatMessage.getSpeaker({ actor: entity })
			: isToken
			? ChatMessage.getSpeaker({ token: entity.document })
		...
		switch (type) {
			case "start":
				content = game.i18n.format(`${NAME}.CONCENTRATOR.Messages.StartConcentration`, {
					entityName: entity.name,
					spellName,
				});
				isWhisper =
					Sidekick.getSetting(SETTING_KEYS.concentrator.notifyConcentration) ===
					Sidekick.getKeyByValue(
						DEFAULT_CONFIG.concentrator.notifyConcentration,
						DEFAULT_CONFIG.concentrator.notifyConcentration.gm
					);
				break;

			case "check":
				content = game.i18n.format(`${NAME}.CONCENTRATOR.Messages.ConcentrationTested`, {
					entityName: entity.name,
					dc,
					spellName,
				});
				isWhisper =
					Sidekick.getSetting(SETTING_KEYS.concentrator.notifyConcentrationCheck) ===
					Sidekick.getKeyByValue(
						DEFAULT_CONFIG.concentrator.notifyConcentrationCheck,
						DEFAULT_CONFIG.concentrator.notifyConcentrationCheck.gm
					);
				break;

			case "dead":
				content = game.i18n.format(`${NAME}.CONCENTRATOR.Messages.Incapacitated`, {
					entityName: entity.name,
					spellName,
				});
				isWhisper =
					Sidekick.getSetting(SETTING_KEYS.concentrator.notifyEndConcentration) ===
					Sidekick.getKeyByValue(
						DEFAULT_CONFIG.concentrator.notifyEndConcentration,
						DEFAULT_CONFIG.concentrator.notifyEndConcentration.gm
					);
				break;

			case "double":
				content = game.i18n.format(`${NAME}.CONCENTRATOR.Messages.DoubleConcentration`, {
					entityName: entity.name,
					oldSpellName: spellName,
					newSpellName: newSpell.name,
				});
				isWhisper =
					Sidekick.getSetting(SETTING_KEYS.concentrator.notifyDouble) ===
					Sidekick.getKeyByValue(
						DEFAULT_CONFIG.concentrator.notifyDouble,
						DEFAULT_CONFIG.concentrator.notifyDouble.gm
					);
				break;

			case "end":
				content = game.i18n.format(`${NAME}.CONCENTRATOR.Messages.EndConcentration`, {
					entityName: entity.name,
					spellName,
				});
				isWhisper =
		...
		const whisperRecipients = isWhisper
			? Concentrator._getWhisperRecipients(
					entity,
					Sidekick.getKeyByValue(
						DEFAULT_CONFIG.concentrator.messageVisibility,
						DEFAULT_CONFIG.concentrator.messageVisibility.gmOwner
					)
			  )
			: [];
	 ...
	 */
	static async _startConcentration(
		entity,
		spell,
		conditionName,
		{ sendMessage = DEFAULT_CONFIG.concentrator.notifyConcentration.none } = {}
	...
	static _shouldSendMessage(eventType) {
		...
		switch (eventType) {
			case "start":
				keyNone = Sidekick.getKeyByValue(
					DEFAULT_CONFIG.concentrator.notifyConcentration,
					DEFAULT_CONFIG.concentrator.notifyConcentration.none
				);
			...
			case "check":
				keyNone = Sidekick.getKeyByValue(
					DEFAULT_CONFIG.concentrator.notifyConcentrationCheck,
					DEFAULT_CONFIG.concentrator.notifyConcentrationCheck.none
				);
			...
			case "double":
				keyNone = Sidekick.getKeyByValue(
					DEFAULT_CONFIG.concentrator.notifyDouble,
					DEFAULT_CONFIG.concentrator.notifyDouble.none
				);
			...
			case "end":
				keyNone = Sidekick.getKeyByValue(
					DEFAULT_CONFIG.concentrator.notifyEndConcentration,
					DEFAULT_CONFIG.concentrator.notifyEndConcentration.none
				);
	...
	static _getWhisperRecipients(entity, desiredVisibility) {
		...
		const visibilityGmOwner = Sidekick.getKeyByValue(
			DEFAULT_CONFIG.concentrator.messageVisibility,
			DEFAULT_CONFIG.concentrator.messageVisibility.gmOwner
		);
		const visibilityAll = Sidekick.getKeyByValue(
			DEFAULT_CONFIG.concentrator.messageVisibility,
			DEFAULT_CONFIG.concentrator.messageVisibility.all
		);
