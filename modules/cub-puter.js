...
export class CUBPuter extends FormApplication {
	...
	getData() {
		...
		currentGadget.id =
			Object.keys(currentGadget).length > 0 ? Sidekick.getKeyByValue(gadgets, currentGadget) : null;
		this.currentGadgetId = this.currentGadgetId ? this.currentGadgetId : currentGadget.id ? currentGadget.id : null;
		const gadgetSettingKeys = hasProperty(SETTING_KEYS, currentGadget.id)
			? Object.values(SETTING_KEYS[currentGadget.id]).map((k) => `${NAME}.${k}`)
			: [];

		const settings = Array.from(game.settings.settings.entries())
			.filter((entry) => {
	...
	async _onRestoreDefaults(event, html) {
		...
		ui.notifications.info(
			`${GADGETS[this.currentGadgetId].name} ${game.i18n.localize("NOTIFICATIONS.CUBPuter.RestoreDefaults")}`
		);
		this.render();
	}
	...
	async _updateObject(event, formData) {
		...
		if (updated.length) {
			ui.notifications.info(
				`${GADGETS[this.currentGadgetId].name} ${game.i18n.localize("NOTIFICATIONS.CUBPuter.SaveSettings")}`
			);
		}
	}
	...
	static _onRender(app, html, data) {
		...
		const greetingOutput = config.greeting
			? `<span id="greetings-output">> ${
					greetings[Math.floor(Math.random() * greetings.length)]
			  } <a id="clear-greeting">--Remove this message?--</a><br /></span>`
			: ``;
		const instructionsPrompt = config.instructions
			? `<span id="instructions-prompt"><br />${username}@cubputer:~$ </span>`
			: ``;
		const instructionsInput = config.instructions ? `<span id="instructions-input">./instructions.sh</span>` : ``;
		const instructionsOutput = config.instructions
			? `<span id="instructions-output"><br />> Select a gadget to get started. You can read more information about the gadget, or change its settings. With our powers combined the players stand no chance! <a id="clear-instructions">--Remove this message?--</a></span><br />`
			: ``;
		...
		const infoPrompt = config.info
			? `<br /> <span class="sudo-prompt">root@cubputer:/gadgets/${app.currentGadgetId}# </span>`
			: ``;
...
export function createCUBPuterButton(html) {
	...
	const cubPuterButton = $(
		`<button id="${DEFAULT_CONFIG.cubPuter.buttonId}" data-action="cub-puter" title="${game.i18n.localize(
			"SETTINGS.CUBPuter.ButtonH"
		)}">
