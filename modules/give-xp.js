...
export class GiveXP {
	...
	static _onRenderDialog(app, html, data) {
		...
		const xpCheckboxGroup = $(
			`<div class="form-group"><label class="xp-checkbox">Award XP? <input type="checkbox" name="award-xp"></label></div>`
		);
	...
	static _distributeDialogRender(html) {
		...
		function updateXp() {
			...
			html.find("#friendly-actor-list label").each((_, el) => {
				...
				$(el)
					.find(".give-xp--actor-xp")
					.text(`+${giveXp ? xpPerDivisor : 0} XP`);
			});
	...
	static async _distributeXP(html, { combat, friendlies }) {
		const getSelectedTokens = (type) =>
			html
				.find(`#${type}-actor-list label`)
				.has("input:checked")
				.map((_, el) => canvas.tokens.get($(el).data("tokenId")))
				.get();
		...
		if (selectedFriendlyTokens.length !== 0 && selectedHostileTokens.length !== 0) {
			const totalXp =
				selectedHostileTokens.reduce((total, token) => total + token.actor.system.details.xp.value, 0) *
				xpModifier;
			...
			let levelUps = selectedFriendlyTokens.filter(
				({ actor }) => actor.system.details.xp.value >= actor.system.details.xp.max
			);
			if (levelUps.length) {
				await this.outputToChat(`
