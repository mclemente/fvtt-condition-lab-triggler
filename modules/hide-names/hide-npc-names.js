...
export class HideNPCNames {
	...
	static _updateEntityMessages(entity) {
		...
		const messages = game.messages.contents.filter(
			(m) => m.logged && m.speaker?.[isToken ? "token" : "actor"] === entity.id
		);
	...
	static _onRenderCombatTracker(app, html, data) {
		...
		const hideNPCs = npcs
			.filter((n) => HideNPCNames.shouldReplaceName(n.actor))
			.map((npc) => {
	...
	static async _onRenderChatMessage(message, html, data) {
		...
		if (game.user.isGM || actor.isOwner) {
			...
			const title = `${
				shouldReplace
					? `${game.i18n.localize(
							`${NAME}.HIDE_NAMES.MessageIcon.Title.NameHiddenPrefix`
					  )} ${replacementName} ${game.i18n.localize(
							`${NAME}.HIDE_NAMES.MessageIcon.Title.NameHiddenSuffix`
					  )}`
					: game.i18n.localize(`${NAME}.HIDE_NAMES.MessageIcon.Title.NameNotHidden`)
			}`;
			const $icon = $(
	...
	static shouldReplaceName(actor) {
		...
		const dispositionEnableSetting = Sidekick.getSetting(
			SETTING_KEYS.hideNames[`enable${disposition.titleCase()}`]
		);
	...
	static getReplacementName(actor) {
		const dispositionEnum = actor.isToken ? actor.token.disposition : actor.prototypeToken.disposition;
		const disposition = Sidekick.getKeyByValue(CONST.TOKEN_DISPOSITIONS, dispositionEnum);
		const replacementSetting = Sidekick.getSetting(
			SETTING_KEYS.hideNames[`${disposition.toLowerCase()}NameReplacement`]
		);
