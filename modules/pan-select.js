...
export class PanSelect {
	...
	static _onPreUpdateCombat(combat, update, options, userId) {
		...
		if (
			...
			getProperty(update, "started") ||
			(getProperty(combat, "round") === 0 && getProperty(update, "round") === 1)
		) {
	...
	static _updateHandler(combatant, type) {
		...
		const temporary = hasProperty(
			combatant,
			`flags.${BUTLER.NAME}.${BUTLER.FLAGS.temporaryCombatants.temporaryCombatant}`
		);
