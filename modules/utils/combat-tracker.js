...
export class TrackerUtility {
	...
	static _onDeleteCombat(combat, options, userId) {
		...
		const tempCombatants = combat.combatants.filter((c) =>
			hasProperty(c, `flags.${BUTLER.NAME}.${BUTLER.FLAGS.temporaryCombatants.temporaryCombatant}`)
		);
	...
	static _onDeleteCombatant(combatant, options, userId) {
		const tempCombatantFlag = combatant?.token?.getFlag(
			BUTLER.NAME,
			BUTLER.FLAGS.temporaryCombatants.temporaryCombatant
		);
